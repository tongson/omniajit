...
local from = function(base, cwd, name)
    ...
    fn.run = function(...)
        local a = pargs(...)
        msg.debug(sf("RUN %s", concat(a, " ")))
        exe("run", name, "--", unpack(a))
    end
    fn.apt_get = function(...)
        local a = pargs(...)
        msg.debug(sf("RUN apt-get %s", concat(a, " ")))
